 ...
 * (`NSAttributedString` on iOS, `SpannableString` on Android, `Inline` on Windows).
 * In practice, this is very tedious. For React Native, we decided to use web 
 * paradigm for this where you can nest text to achieve the same effect.
 *
 *
 * ```ReactNativeWebPlayer
 * import React, { Component } from 'react';
 ...
 * ```
 *
 * ## Nested views (iOS and Windows only)
 *
 * On iOS and Windows, you can nest views within your Text component. Here's an example:
 *
 * ```ReactNativeWebPlayer
 * import React, { Component } from 'react';
